...
const RelatedEngr = ({engrId, speciality}) => {

    const {engineers}= useContext(AppContext)
    const navigate = useNavigate()

    const [relEngr, setRelEngr] = useState([])

    useEffect(()=>{
        if (engineers.length > 0 && speciality){
            const engineerData = engineers.filter((engr)=> engr.speciality === speciality && engr._id !== engrId)
            setRelEngr(engineerData)
        }

    },[engineers, speciality, engrId])
  return (
    <div className='flex flex-col items-center gap-4 my-16 text-white md:mx-10'>
    <h1 className='text-3xl font-medium'>Related <span className='text-orange-500'>Engineers</span> To <span className='text-orange-500'>Book</span></h1>
    <p className='sm:w-1/3 text-center text-sm'>Simply browse  through our extensive list of trusted engineers.</p>
    <div className='w-full grid grid-cols-auto gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
        {relEngr.slice(0,4).map((item,index)=>(
